...
namespace ComplianceRecordingBot.FrontEnd.MediaBuffer
{
    ...
    public abstract class BufferBase<T>
    {
        ...
        public async Task Append(T obj)
        {
            ...
            {
                Buffer?.Complete();
                Debug.Write($"Cannot enqueue because queuing operation has been cancelled. error: {e}");
            }
        }
        ...
        private async Task _process()
        {
            ...
            {
                Debug.Write(string.Format("The queue processing task has been cancelled. TaskCanceledException: {0}", ex));
            }
            catch (ObjectDisposedException ex)
            {
                Debug.Write(string.Format("The queue processing task object has been disposed. ObjectDisposedException: {0}", ex));
            }
            catch (Exception ex)
            {
                // Catch all other exceptions and log
                Debug.Write(string.Format("Caught error: {0}", ex));
